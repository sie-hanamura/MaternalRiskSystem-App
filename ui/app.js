...
function displayResults(result) {
    ...
    populateProbabilityDetails(result);
    populateFeatureAnalysis(result);
    // ✨✨✨ END OF ADDITION ✨✨✨
...
// ✨✨✨ PASTE EXPLANATION FUNCTIONS HERE ✨✨✨
// Toggle explanation panel
function toggleExplanation() {
    const content = document.getElementById('explanation-content');
    const icon = document.getElementById('toggle-icon');
    const header = document.querySelector('.explanation-header');  // ✅ Add this
    
    content.classList.toggle('expanded');
    icon.classList.toggle('expanded');
    header.classList.toggle('expanded');  // ✅ Add this
}

// Switch between explanation tabs
function switchExplanationTab(tabName) {
    document.querySelectorAll('.tab-content').forEach(tab => {
        tab.classList.remove('active');
    });
    
    document.querySelectorAll('.explanation-tab').forEach(btn => {
...
// Populate probability details dynamically
function populateProbabilityDetails(result) {
    const container = document.getElementById('probability-details');
    const probs = result.probabilities;
    const predicted = result.risk_level;
    
    const riskLevels = [
        { key: 'low', name: 'Low Risk', color: '#10b981', icon: '✅' },
        { key: 'moderate', name: 'Moderate Risk', color: '#f59e0b', icon: '⚠️' },
        { key: 'high', name: 'High Risk', color: '#ef4444', icon: '🚨' }
    ];
    
    let html = '';
    
    riskLevels.forEach(level => {
        const isPredicted = predicted === level.name.split(' ')[0];
        const probability = probs[level.key];
        
        html += `
            <div class="probability-detail ${isPredicted ? 'highlighted' : ''}">
                <div class="probability-header">
                    <div>
                        <span class="probability-name">${level.icon} ${level.name}</span>
                        ${isPredicted ? '<span class="star-badge">⭐ PREDICTED</span>' : ''}
                    </div>
                    <span class="probability-value" style="color: ${level.color};">${probability.toFixed(1)}%</span>
                </div>
                <div class="probability-bar">
                    <div class="probability-bar-fill" style="width: ${probability}%; background: ${level.color};"></div>
                </div>
                <p class="probability-description">
                    ${getExplanationText(level.key, isPredicted, probability)}
                </p>
            </div>
        `;
    });
...
// Get explanation text for each probability level
function getExplanationText(level, isPredicted, probability) {
    if (level === 'low') {
        if (isPredicted) {
            return `<strong>Highest probability.</strong> The model is ${probability.toFixed(1)}% confident this pregnancy falls into the low-risk category requiring routine prenatal care only.`;
        }
        return `The model assigns a ${probability.toFixed(1)}% probability to the low-risk category.`;
    } else if (level === 'moderate') {
        if (isPredicted) {
            return `<strong>Highest probability.</strong> The model is ${probability.toFixed(1)}% confident this pregnancy requires enhanced monitoring.`;
        }
        if (probability > 30) {
            return `There's a substantial probability (${probability.toFixed(1)}%) that this pregnancy could be moderate-risk.`;
        }
        return `The model assigns a ${probability.toFixed(1)}% probability to moderate risk.`;
    } else {
        if (isPredicted) {
            return `<strong>Highest probability.</strong> The model is ${probability.toFixed(1)}% confident this pregnancy requires immediate specialist referral.`;
        }
        if (probability > 10) {
            return `There's a notable probability (${probability.toFixed(1)}%) of high risk.`;
        }
        return `Very low probability (${probability.toFixed(1)}%) that this pregnancy requires immediate specialist referral.`;
    }
}

// Populate feature analysis dynamically
function populateFeatureAnalysis(result) {
    const container = document.getElementById('feature-analysis');
    const bmi = parseFloat(result.bmi);
    const systolic = parseFloat(document.getElementById('systolic').value);
    const diastolic = parseFloat(document.getElementById('diastolic').value);
    const bloodSugar = parseFloat(document.getElementById('blood-sugar').value);
    const hemoglobin = parseFloat(document.getElementById('hemoglobin').value);
    const labAvailable = result.lab_available;
    
    let features = [];
    
    if (labAvailable) {
        features = [
            { name: 'BMI (Body Mass Index)', value: bmi.toFixed(1) + ' kg/m²', contribution: 35, status: getBMIStatus(bmi), explanation: getBMIExplanation(bmi) },
            { name: 'Systolic Blood Pressure', value: systolic + ' mmHg', contribution: 30, status: getBPStatus(systolic, 'systolic'), explanation: getBPExplanation(systolic, 'systolic') },
            { name: 'Blood Sugar Level', value: bloodSugar.toFixed(1) + ' mmol/L', contribution: 20, status: getBloodSugarStatus(bloodSugar), explanation: getBloodSugarExplanation(bloodSugar) },
            { name: 'Hemoglobin Level', value: hemoglobin.toFixed(1) + ' g/dL', contribution: 10, status: getHemoglobinStatus(hemoglobin), explanation: getHemoglobinExplanation(hemoglobin) },
            { name: 'Diastolic Blood Pressure', value: diastolic + ' mmHg', contribution: 5, status: getBPStatus(diastolic, 'diastolic'), explanation: getBPExplanation(diastolic, 'diastolic') }
        ];
    } else {
        features = [
            { name: 'BMI (Body Mass Index)', value: bmi.toFixed(1) + ' kg/m²', contribution: 50, status: getBMIStatus(bmi), explanation: getBMIExplanation(bmi) },
            { name: 'Systolic Blood Pressure', value: systolic + ' mmHg', contribution: 30, status: getBPStatus(systolic, 'systolic'), explanation: getBPExplanation(systolic, 'systolic') },
            { name: 'Diastolic Blood Pressure', value: diastolic + ' mmHg', contribution: 20, status: getBPStatus(diastolic, 'diastolic'), explanation: getBPExplanation(diastolic, 'diastolic') }
        ];
    }
            ...
            <div class="feature-item">
                <div class="feature-header">
                    <div>
                        ...
                        <div class="feature-value">Measured value: <strong>${feature.value}</strong></div>
                    </div>
                    <span class="feature-status ${feature.status.class}">${feature.status.icon} ${feature.status.text}</span>
                </div>
                <div class="contribution-bar">
                    <div class="contribution-label">
                        <span style="color: #6b7280;">Contribution to prediction</span>
                        <span style="color: #36ABA3; font-weight: 700;">${feature.contribution}%</span>
                    </div>
                    <div class="contribution-track">
                        <div class="contribution-fill" style="width: ${feature.contribution}%"></div>
                    </div>
                </div>
                ${isPrimary ? `<div class="primary-factor-badge"><strong>⭐ Primary risk factor:</strong> ${feature.explanation}</div>` : ''}
            </div>
        `;
    });
...
function getBMIStatus(bmi) {
    if (bmi < 18.5) return { class: 'warning', icon: '⚠', text: '<18.5 (Underweight)' };
    if (bmi < 25) return { class: 'normal', icon: '✓', text: '18.5-24.9 (Normal)' };
    if (bmi < 30) return { class: 'warning', icon: '⚠', text: '25-29.9 (Overweight)' };
    return { class: 'danger', icon: '⚠', text: '≥30 (Obese)' };
}

function getBMIExplanation(bmi) {
    if (bmi < 18.5) return 'Low BMI indicates underweight status.';
    if (bmi < 25) return 'Normal BMI contributes positively to the prediction.';
    if (bmi < 30) return 'Elevated BMI suggests overweight status.';
    return 'High BMI is a major risk factor.';
}

function getBPStatus(value, type) {
    if (type === 'systolic') {
        if (value < 120) return { class: 'normal', icon: '✓', text: '<120 (Normal)' };
        if (value < 140) return { class: 'warning', icon: '⚠', text: '120-139 (Elevated)' };
        return { class: 'danger', icon: '⚠', text: '≥140 (Hypertensive)' };
    } else {
        if (value < 80) return { class: 'normal', icon: '✓', text: '<80 (Normal)' };
        if (value < 90) return { class: 'warning', icon: '⚠', text: '80-89 (Elevated)' };
        return { class: 'danger', icon: '⚠', text: '≥90 (Hypertensive)' };
    }
}

function getBPExplanation(value, type) {
    const label = type === 'systolic' ? 'Systolic' : 'Diastolic';
    if ((type === 'systolic' && value >= 140) || (type === 'diastolic' && value >= 90)) {
        return `${label} BP is elevated.`;
    }
    if ((type === 'systolic' && value >= 120) || (type === 'diastolic' && value >= 80)) {
        return `${label} BP is prehypertensive.`;
    }
    return `${label} BP is normal.`;
}

function getBloodSugarStatus(value) {
    if (value < 5.6) return { class: 'normal', icon: '✓', text: '<5.6 (Normal)' };
    if (value < 7.0) return { class: 'warning', icon: '⚠', text: '5.6-6.9 (Prediabetic)' };
    return { class: 'danger', icon: '⚠', text: '≥7.0 (Diabetic)' };
}

function getBloodSugarExplanation(value) {
    if (value >= 7.0) return 'Elevated blood sugar indicates gestational diabetes.';
    if (value >= 5.6) return 'Blood sugar is in prediabetic range.';
    return 'Blood sugar level is normal.';
}

function getHemoglobinStatus(value) {
    if (value >= 11.0) return { class: 'normal', icon: '✓', text: '≥11.0 (Normal)' };
    if (value >= 9.5) return { class: 'warning', icon: '⚠', text: '9.5-10.9 (Mild Anemia)' };
    return { class: 'danger', icon: '⚠', text: '<9.5 (Severe Anemia)' };
}

function getHemoglobinExplanation(value) {
    if (value < 9.5) return 'Low hemoglobin indicates severe anemia.';
    if (value < 11.0) return 'Hemoglobin is slightly low.';
    return 'Hemoglobin level is adequate.';
}
// ✨✨✨ END OF EXPLANATION FUNCTIONS ✨✨✨